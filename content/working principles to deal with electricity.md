...
# capacitor:
a capacitor also has two legs (a '+' and a '-') like a battery. however, it doesn't have little beings of its own — so it steals them. when beings move from '+' to '-' in a circuit, the capacitor traps some in a special chamber where the energy of these beings remains the same — but they can't move anywhere. then, when traffic outside reduces, the capacitor lets some of them out. the being travels to the capacitor's '-' side for a few times, before realizing that its not the power source. 

---

[^1]: this is actually untrue. electrons actually move in the opposite direction (from '-' to '+'). see this [wiki-image](https://upload.wikimedia.org/wikipedia/commons/thumb/e/e9/Current_Flow.svg/500px-Current_Flow.svg.png?utm_source=commons.wikimedia.org&utm_campaign=index&utm_content=thumbnail&_=20060522153928), but it is easier for our brains to think that they move from '+' to '-'. this is referred to as 'conventional current flow'. read more [here](https://en.wikipedia.org/wiki/Electric_current).
